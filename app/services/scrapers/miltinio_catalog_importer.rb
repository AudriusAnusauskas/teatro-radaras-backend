module Scrapers
  class MiltinioCatalogImporter < BaseCatalogImporter
    ...
    def import_one(data)
      ...
      unless data[:title].present?
        Rails.logger.warn("[MiltinioCatalogImporter] Skipping #{data[:source_url]} — no title")
        return [:skipped, 0]
      end
      ...
      if is_new || production.director_id.blank?
        if data[:director].blank?
          Rails.logger.warn("[MiltinioCatalogImporter] SKIP (no director): #{data[:slug]}")
          return [:skipped, 0]
        end
    ...
    def production_attributes(_data, _director)
      raise "MiltinioCatalogImporter uses assign_scraped_attributes instead"
    end
  end
end
